...
const useAttributes = ({
  ...
  const initialState: (attribute: number | string) => {} = (attribute) => {
    return {
      value: attribute,
      renderStatEditor: <span data-testid="rendered-stat">{attribute}</span>, //Starts out as span and toggles to <StatEditor/>
    };
  };
    ...
    switch (action.type) {
      case "editOn":
        return {
          ...
          renderStatEditor: (
            <StatEditor
              data-testid="stat-editor"
              originalStat={state.value}
      ...
      case "editOff":
        return {
          ...state,
          renderStatEditor: (
            <span data-testid="rendered-stat">{action.payload}</span>
          ),
